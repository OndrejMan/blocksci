...
namespace blocksci {
    namespace heuristics {
        ...
        bool isCoinjoin(const Transaction &tx) {
            return isAshigaruCoinJoin(tx) || isWhirlpoolCoinJoin(tx) || isWasabi2CoinJoin(tx) ||
                   isWasabi1CoinJoin(tx) || isJoinMarketCoinJoin(tx);
        }
        ...
        bool isWasabi2CoinJoin(const Transaction &tx, std::optional<uint64_t> inputCount, bool debug) {
            ...
            if ((inputCount.has_value() && tx.inputCount() < inputCount.value()) ||
            ...
            if (usedAddresses.size() + usedWitnessUnknownAddresses.size() < 5) {
                if (debug) {
                    std::cout << "Input addresses: " << usedAddresses.size() << " + "
                              << usedWitnessUnknownAddresses.size() << std::endl;
                }
                return false;
            }
            ...
            if (usedAddresses.size() + usedWitnessUnknownAddresses.size() < 5) {
                if (debug) {
                    std::cout << "Output addresses: " << usedAddresses.size() << " + "
                              << usedWitnessUnknownAddresses.size() << std::endl;
                }
                return false;
            }
            ...
            if (debug) {
                std::cout << "Output count: " << count << ", " << tx.outputCount() * (blockHeight < 850237 ? 0.8 : 0.75)
                          << std::endl;
            }
        ...
        bool isWhirlpoolCoinJoin(const Transaction &tx) {
            ...
            // tx.Inputs.Where(x => x.PrevOutput.Value != poolSize).All(x => x.PrevOutput.Value.Almost(poolSize,
            // Money.Coins(0.0011m)));
            return std::all_of(tx.inputs().begin(), tx.inputs().end(), [current_pool_size](const Input &input) {
                return input.getValue() == current_pool_size || std::abs(input.getValue() - current_pool_size) < 110000;
            });
        }

        bool isAshigaruCoinJoin(const Transaction &tx) {
            if (tx.getBlockHeight() < blocksci::CoinjoinUtils::FirstAshigaruBlock) {
                return false;
            }
        ...
        bool isJoinMarketCoinJoin_100k(const Transaction &tx) {
            ...
            auto most_frequent_output = std::max_element(
                output_values.begin(), output_values.end(),
                [](const std::pair<int64_t, int> &a, const std::pair<int64_t, int> &b) { return a.second < b.second; });
            ...
            if (tx.outputCount() > 2 * n) {
                return false;
            }

            std::unordered_set<Address> input_scripts;
            for (const auto &input : tx.inputs()) {
                input_scripts.insert(input.getAddress());
            }

            if (input_scripts.size() < n) {
                return false;
            }

            std::unordered_set<Address> output_scripts;
            for (const auto &output : tx.outputs()) {
                output_scripts.insert(output.getAddress());
            }
        ...
        CoinJoinType getCoinjoinTag(const Transaction &tx) {
            if (blocksci::heuristics::isWasabi2CoinJoin(tx)) {
                ...
                return blocksci::heuristics::CoinJoinType::Ashigaru;
            } else if (blocksci::heuristics::isWasabi1CoinJoin(tx)) {
                return blocksci::heuristics::CoinJoinType::WW1;
            } else if (blocksci::heuristics::isJoinMarketCoinJoin(tx)) {
                return blocksci::heuristics::CoinJoinType::JoinMarket;
            } else {
