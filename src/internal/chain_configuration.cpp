...
namespace blocksci {
    ...
    void to_json(json& j, const CoinJoinConfiguration &p) {
        j["coinJoinConfiguration"] = {
            ...
            {"FirstAshigaruBlock", p.FirstAshigaruBlock}
        };
    }
    ...
    void from_json(const json& j, CoinJoinConfiguration &p) {
        ...
        // Added after the initial coinJoinConfiguration block shipped, so configs
        // generated by an older parser do not carry it yet.
        auto ashigaru = j.find("FirstAshigaruBlock");
        if (ashigaru != j.end()) {
            ashigaru->get_to(p.FirstAshigaruBlock);
        } else {
            p.FirstAshigaruBlock = CoinjoinUtils::DefaultFirstAshigaruBlock;
        }
    }
    ...
    ChainConfiguration ChainConfiguration::bitcoinRegtest(const std::string &chainDir) {
        return {
            ...
            "bcrt",
            0, 
            CoinJoinConfiguration{0,0,0,0,0}  // Set all block heights to 0 and search whole regtest
        };
    }
    ...
    ChainConfiguration ChainConfiguration::bitcoin(const std::string &chainDir) {
        return {
            ...
            "bc",
            481824,
            // SW, WW1, WW2, WW1 no static coordinator, Ashigaru
            CoinJoinConfiguration{570000, 530500, 741213, 610000, CoinjoinUtils::DefaultFirstAshigaruBlock}
        };
    }
    ...
    ChainConfiguration ChainConfiguration::bitcoinTestnet(const std::string &chainDir) {
        return {
            ...
            "tb",
            834624, 
            CoinJoinConfiguration{0, 0, 0, 0, 0}  // Set all block heights to 0 and search whole testnet
        };
    }
